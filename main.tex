...
\begin{enumerate}
    ...
    \item We construct a \textsc{Mentor}-grounded training corpus that supports four task cases: partial group completion, mechanism explanation, noisy group refinement, and recognition of no shared mechanism (the agent abstains or separates input into multiple functional groups).
...
\midrule
\texttt{shortest\_paths} & Compute shortest paths between genes across the multiplex or a selected set of layers, returning paths with layer provenance & \texttt{source\_genes}: list of gene identifiers; \texttt{target\_genes}: list of gene identifiers; \texttt{layers}: list of layer names (optional; defaults to all) & DPO, RL \\
\midrule
\texttt{rwr} & Run random walk with restart from one or more seed genes over the multiplex (optionally restricted to selected layers) and return ranked node scores with provenance & \texttt{seed\_genes}: list of gene identifiers; \texttt{layers}: list of layer names (optional; defaults to all); \texttt{top\_k}: number of results to return & DPO, RL \\
\midrule
\texttt{rwr\_loe} & Rank a set of query genes by their random-walk-with-restart affinity to the seed genes, supporting relatedness and refinement decisions & \texttt{seed\_genes}: list of gene identifiers; \texttt{query\_genes}: list of gene identifiers; \texttt{top\_k}: number of results to return & DPO, RL \\
\midrule
...
\subsubsection{MENTOR-Derived Training Corpus}\label{sec:mentor-corpus}
...
Each \textsc{Mentor}-derived task instance is paired with a graph environment defined over the same canonical gene universe as the in-house HumanNet brain multiplex, of which our \textsc{Mentor} modules are a labeled subset. The sampled input set $C^{\text{in}}$ initializes the agent's graph anchors, while the hidden target $C$ defines the supervisory signal.
...
\begin{equation}\label{eqn:local-reward}
    s^{local}_{t,i,j} = w_s\, s^{schema} + w_c\, \Delta s^{complex} + w_m\, \Delta s^{mech} - w_e\, p^{eff} - p^{mismatch}
\end{equation}
...
\]
providing a balanced view of precision and recall to measure general performance. In addition to these complex recovery metrics, we will measure mechanistic grounding quality, defined in Equation~\ref{eqn:terminal-mech-abs}, to assess whether mechanistic claims are supported by retrieved evidence.  
