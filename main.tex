...
\begin{enumerate}
    ...
    \item We construct a \textsc{Mentor}-grounded training corpus that supports five task cases: partial group completion, mechanism explanation, noisy group refinement, and recognition of no shared mechanism (the agent abstains or separates input into multiple functional groups).
    ...
    \item We will release the training environment, \textsc{Mentor} split indices, evaluation harness, and trained checkpoints, and we will benchmark against frontier and open models under both empirical and blind expert-preference evaluation.
\end{enumerate}
...
\midrule
\texttt{enrich\_gene\_set} & Run functional enrichment over a candidate gene set against a background universe and return ranked enriched terms with significance & \texttt{genes}: list of gene identifiers; \texttt{background}: background gene set; \texttt{top\_k}: number of terms to return; \texttt{user\_threshold}: significance cutoff in $(0,1]$ & DPO, RL \\
\bottomrule
\end{tabularx}
\end{table}
...
The runtime uses a set of optimized multiplex functions developed as part of the \textsc{\textbf{Mentor}} \cite{mentor} and \textsc{\textbf{Mentor-IA}} \cite{mentor-ia} projects, referred to as RWR++. Core operators include neighborhood retrieval, shortest-path search, random walk with restart on both monoplex and multiplex views, and subgraph induction. These operators provide the primary graph-facing actions available to the agent during exploration.
...
\begin{enumerate}
    \item the \textsc{Mentor}-grounded training environment, including the \texttt{ctypes} multiplex runtime, tool vocabulary, and deterministic scoring functions,
