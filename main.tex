...
While preference optimization provides a solution for aligning LLMs with preferences, policy gradient and group-relative methods provide the infrastructure for parameter updates across a variety of tasks. Proximal policy optimization (PPO) \cite{schulman2017proximal} was introduced as a simple, efficient, and scalable alternative to previous methods, \cite{schulman2015trust}, using a clipped probability ration term to form a pessimistic estimate of performance and KL divergence term to promote stable optimization. Group relative policy optimization (GRPO) \cite{shao2024deepseekmath} built upon PPO, computing the advantage term by measuring the divergence of outputs from a group mean rather than using a trained value model, reducing the memory burden of training. DeepSeek-R1 \cite{guo2025deepseek} used GRPO at the frontier scale, showing that state-of-the-art LLM training could be achieved using the GRPO algorithm. The power of policy gradient methods, especially over many reasoning steps, makes them conducive to \textbf{\textsc{Mentor-RL}} training, where long-horizon reinforcement learning is necessary for multi-hop biological graph exploration.

\subsubsection{RL for Tool-Augmented Agents}
\begin{itemize}
    \item WebGPT — Nakano et al. 2021 (RLHF + tool use, foundational)
    \item ToolLLM — Qin et al. 2024 (SFT baseline; DFSDT tree search at inference)
    \item Agent Q — Putta et al. 2024 (MCTS + self-critique + off-policy DPO; closest precedent)
    \item Beyond Human Data / ReST$^{EM}$ — Singh et al. 2024 (verifier-grounded self-training)
    \item Search-R1 / R1-Searcher — 2025 (GRPO with search tools; recent)
\end{itemize}
