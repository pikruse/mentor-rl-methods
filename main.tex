...
We define two complementary target types for training and evaluation. \textbf{Module membership targets} are the hidden gene sets of \textsc{Mentor}-derived dendrogram modules (Section~\ref{sec:data}). These targets are only used for scoring: the agent never observes hidden module members and must recover them through exploration. These modules supply the supervised signal for set-level recovery, refinement, and abstention. 
...
\For{each module $C_{\text{module}}$ with size $n$ and difficulty map $\delta$}
