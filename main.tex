...
To make the warm-start SFT stage concrete, we define a core tool vocabulary derived from the existing \textbf{\textsc{Mentor-IA}} infrastructure~\cite{mentor-ia} in Table~\ref{tbl:tools}. Early SFT emphasizes schema-valid biological annotation and enrichment calls, while graph-exploration and RWR++ operators are introduced during DPO and online RL.

\begin{table}[p]
\centering
\caption{Tool vocabulary for the \textbf{\textsc{Mentor-RL}} agent. Related RWR++ calls are grouped by function while preserving the model-facing tool names.}
\label{tbl:tools}
\scriptsize
...
Each \textsc{Mentor}-derived task instance is paired with a graph environment defined over the same canonical gene universe as the in-house brain multiplex, from which our \textsc{Mentor} modules are derived. The sampled input set $C^{\text{in}}$ initializes the agent's graph anchors, while the hidden target $C$ defines the supervisory signal.
...
We scale long-horizon reinforcement learning difficulty with several strategies. First, we modulate the trajectory length $T_{\max}$, starting with shorter exploration budgets and gradually increasing exploration length for problems that require multi-hop exploration. Additionally, we modulate module size, beginning training on small modules and shifting to larger ones as rewards saturate. Finally, we scale the per-task difficulty level, which jointly controls the number of dropped and added genes and the dendrogram-distance band from which noise and \texttt{none} genes are drawn (Section~\ref{sec:mentor-corpus}): higher difficulties remove or add more genes and draw distractors from nearer, more easily confused regions of the dendrogram, making recovery and refinement harder.

\subsection{Evaluation and Benchmarks}\label{sec:eval}
\subsubsection{MENTOR Module Holdout Evaluation}
We will use several metrics to evaluate the effectiveness of our model. Each metric will be calculated by parsing the model's terminal structured state $S_T$, allowing for scalable evaluation of our checkpoints. All evaluations will be conducted on a held out set of \textsc{Mentor}-derived modules from the brain multiplex.
