...
Several techniques make use of experimental data to generate and explore networks across multiple interaction layers. Iterative random forest (iRF) expands on traditional random forests by weighting the feature sampling distribution according to variable importance scores across successive fits and using a bootstrapping selection procedure to extract stable, nonlinear interactions \cite{basu2018iterative}. iRF has been used to construct predictive expression networks (PENs), which map the interactions between expressed genes, uncovering true connections from massive datasets that may be missed in hand-curated databases \cite{cliff2019high, walker2022evaluating}. Because biological systems comprise many complementary interaction modalities, integration them without losing information motivated the development on the multiplex network formalism \cite{mucha2010community, de2013mathematical}, which represents the same node set across multiple edge layers.  multiplex networks still support network propagation techniques, such as random walk with restart (RWR) \cite{kohler2008walking, cowen2017network} and other community detection techniques built on the modularity objective \cite{newman2004finding}, most prominently the Louvain method \cite{blondel2008fast} to discover functional modules. More recently, the power of graph deep learning has also been realized, supplanting hand-engineered propagation techniques with learned ones. Graph convolutional networks \cite{kipf2016semi} aggregate neighbor features through message passing, while graph attention networks learn pairwise weights through self attention \cite{velivckovic2017graph}. These techniques enable end-to-end supervised learning on tasks such as node classification, link prediction, and disease gene prioritization, with performance improving on as data availability increases.
...
\begin{itemize}
    ...
    \item ReAct \cite{yao2022react}
    \item PAL \cite{gao2023pal}
    \item Gorilla \cite{patil2024gorilla}
    \item Yao et al. 2023 Tree-of-Thoughts
    \item Shinn et al. 2023 Reflexion
\end{itemize}

\subsubsection{Agentic Architectures with Verification}
\begin{itemize}
    \item Madaan et al. 2023 Self-Refine
    \item Shinn et al. 2023 Reflexion
    \item Asai et al. 2023 Self-RAG
    \item Eubiota \cite{eubiota}
\end{itemize}

\subsubsection{Agentic Gene Set Interpretation}
\begin{itemize}
    \item Eubiota \cite{eubiota}
    \item Mentor-ia \cite{mentor-ia}
    \item GeneAgent (Wang et al. 2024 if applicable),
    \item GenePT (Chen \& Zou 2024) 
\end{itemize}

\subsection{Reinforcement Learning for Long-Horizon Tool-Use}
\subsubsection{Preference Optimization}
\begin{itemize}
    \item Christiano et al. 2017 (RLHF original)
    \item Ouyang et al. 2022 (InstructGPT)
    \item Rafailov et al. 2023 DPO
    \item Azar et al. 2024 IPO (DPO theoretical critique)
    \item Hong et al. 2024 ORPO
\end{itemize}

\subsubsection{Policy Gradient and Group-Relative Methods}
\begin{itemize}
    \item Schulman et al. 2017 PPO
    \item Shao et al. 2024 GRPO
    \item DeepSeek-R1 2025 
\end{itemize}

\subsubsection{RL for Tool-Augmented Agents}
\begin{itemize}
    \item RL for Tool-Augmented Agents — Putta et al. 2024
    \item Agent-Q, Qin et al. 2024
    \item ToolLLM, Singh et al. 2024 Beyond Human Data
\end{itemize}
